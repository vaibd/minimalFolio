const Landing = () => {
  return (
    <div className='text-center content-z-index h-screen pt-[18%] max-md:pt-[6rem] max-[854px]:pt-[4rem] max-[375px]:pt-[1rem]'>
    <div className='content-z-index relative'>
      <h1 className='text-9xl max-md:text-6xl font-bold mb-4'>
        Hi, I'm Vaibhav.
      </h1>
      <h1 className='text-7xl max-md:text-4xl font-bold mb-4'>
        A Front End Engineer.
      </h1>
      <p className='mt-14 max-md:mt-8 text-3xl max-md:text-xl'>
        As a front end engineer, my expertise lies in creating (sometimes
        designing) outstanding digital interfaces.
      </p>
    </div>
  </div>
  );
};

export default Landing;